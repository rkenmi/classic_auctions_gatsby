...
import {
  ...
  searchOnSetSort,
  convertSortParamsToURLParams, getMarketpriceData, hideGraphModal, setTimespanOnGraph
...
class AuctionHouse extends React.Component{
      ...
      <Navbar variant="dark" style={{display: 'flex', paddingLeft: 0, paddingRight: 0}}>
        ...
        <Nav style={{flex: 1, justifyContent: 'flex-end'}}>
          ...
          <RealmDropdown currentRealm={currentRealm}
                         ...
                         onSelectRealmAndFaction={this.props.setCurrentRealmAndFactionAndSearch.bind(this, this.props.location)}
                         realms={realms}/>
        </Nav>
      </Navbar>
    );
  ...
  render() {
    const {loading, items, page, count, graph, noLayout, children, location} = this.props;
...
function mapDispatchToProps(dispatch) {
  return {
    setCurrentRealmAndFactionAndSearch: (location, realm, faction) => {
      dispatch(searchOnSetRealmAndFaction(realm, faction, location.pathname));
    },
